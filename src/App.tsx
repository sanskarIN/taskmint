...
interface ToastState { message: string; actionLabel?: string; action?: () => Promise<void>; }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const check = () => { notifiedIds.current = notifyDueTasks(tasks, notifiedIds.current); };
    check();
  ...
  async function toggleTask(task: Task) {
    ...
    setToast({ message: result.next ? 'Task completed. Next occurrence created.' : 'Task completed.' });
  }
  ...
  async function move(task: Task, direction: -1 | 1) {
    const active = tasks.filter((item) => item.status === 'active').sort((a, b) => a.order - b.order);
    ...
    setTasks((current) => current.map((item) => item.id === moved.id ? moved : item.id === swapped.id ? swapped : item));
  }

  async function dropOn(target: Task) {
    const source = draggedTask.current;
    draggedTask.current = null;
    if (!source || source.id === target.id || source.status !== 'active' || target.status !== 'active') return;
    ...
    const changed = reordered.map((task, index) => ({ ...task, order: index * 1000, updatedAt: now }));
  ...
  async function enableNotifications() {
    ...
    setToast({ message: enabled ? 'Browser reminders enabled.' : 'Notifications were not enabled.' });
  }
  ...
  async function deleteAllData() {
    if (!window.confirm('Delete every TaskMint task and local setting from this browser? This cannot be undone unless you have a backup.')) return;
  ...
  if (loading) {
    return <main className="loading-state" aria-busy="true"><img src="/taskmint-icon.svg" width="56" height="56" alt="" /><p>Loading your local tasks…</p></main>;
  }
    ...
    <div className="app-shell">
      ...
      <header className="topbar">
        <a className="brand" href="/" aria-label="TaskMint home">
          ...
          <span><strong>{strings.appName}</strong><small>{strings.tagline}</small></span>
        </a>
        <div className="top-actions">
          {offline && <span className="offline-badge">Offline</span>}
          <button className="secondary" type="button" onClick={() => setStatsOpen((value) => !value)}>{strings.statistics}</button>
          <button className="secondary" type="button" onClick={() => setSettingsOpen(true)}>{strings.settings}</button>
        </div>
      </header>
      ...
      <div className="workspace">
        <Sidebar
          ...
          onView={chooseView}
          onProject={(project) => setFilters((current) => ({ ...current, project, view: 'all' }))}
        />

        <main className="main-content">
          <TaskComposer editingTask={editingTask} onSubmit={saveDraft} onCancelEdit={() => setEditingTask(null)} />
          <Toolbar
          ...
          <section className="task-section" aria-labelledby="task-list-title">
            <div className="section-heading"><h1 id="task-list-title">{viewTitle(filters)}</h1><span>{visibleTasks.length} {visibleTasks.length === 1 ? 'task' : 'tasks'}</span></div>
            {visibleTasks.length === 0 ? (
              <div className="empty-state card"><h2>{strings.emptyTitle}</h2><p>{strings.emptyBody}</p></div>
            ) : (
              <ul className="task-list">
                {visibleTasks.map((task) => (
                  <TaskItem
                    key={task.id}
                    task={task}
                    isOverdue={task.status === 'active' && !!task.dueDate && task.dueDate < today}
                    onToggle={toggleTask}
                    onEdit={(next) => { setEditingTask(next); window.scrollTo({ top: 0, behavior: settings.reduceMotion ? 'auto' : 'smooth' }); }}
                    onArchive={archive}
                    onRestore={restore}
                    onDelete={remove}
                    onMove={move}
                    onDragStart={(next) => { draggedTask.current = next; }}
                    onDrop={dropOn}
                  />
                ))}
              </ul>
      ...
      <footer className="footer"><span>{strings.madeBy}</span><a href="https://github.com/sanskarIN/taskmint" target="_blank" rel="noreferrer">Source code</a><a href="https://buymeacoffee.com/sanskarIN" target="_blank" rel="noreferrer">Buy Me a Coffee</a></footer>

      {toast && <div className="toast" role="status" aria-live="polite"><span>{toast.message}</span>{toast.action && <button type="button" onClick={() => void toast.action?.()}>{toast.actionLabel ?? 'Undo'}</button>}</div>}
      ...
      <SettingsDialog
        ...
        onExportJson={() => downloadText(`taskmint-backup-${today}.json`, serializeBackup(tasks, settings), 'application/json')}
        onExportCsv={() => downloadText(`taskmint-tasks-${today}.csv`, tasksToCsv(tasks), 'text/csv;charset=utf-8')}
...
function viewTitle(filters: TaskFilters): string {
  ...
  const labels: Record<SmartView, string> = {
    inbox: 'Inbox', today: 'Today', upcoming: 'Upcoming', overdue: 'Overdue', completed: 'Completed', archived: 'Archived', all: 'All tasks'
  };
